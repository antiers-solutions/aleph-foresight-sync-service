// import { Kafka, EachMessagePayload } from 'kafkajs'; // Import necessary types from KafkaJS
...
//          Events.findOne = jest.fn().mockResolvedValue({ eventId: 'event1' });
//          Currency.findOne = jest.fn().mockResolvedValue({ price: 100 });
...
//             ),
//          };
//          await mockEachMessage({ topic: 'syncService', partition: 0, message });

//          // expect(consumerMock.connect).toHaveBeenCalled();
//          // expect(consumerMock.subscribe).toHaveBeenCalledWith({ topic: 'syncService' });
